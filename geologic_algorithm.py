    ...
    def postProcessLayer(self, layer, context, feedback):
        if self.layer_type == 'lines':
            ...
            sym_start = QgsMarkerSymbol.createSimple({
                'name': 'triangle', 'color': '0,255,0,255',
                'outline_color': '0,0,0,255', 'size': '3.2'
            })
            sym_end = QgsMarkerSymbol.createSimple({
                'name': 'square', 'color': '255,0,0,255',
                'outline_color': '0,0,0,255', 'size': '3.2'
            })
            sym_normal = QgsMarkerSymbol.createSimple({
                'name': 'circle', 'color': '100,255,100,255',
                'outline_color': '0,100,0,255', 'size': '2.5'
            })
            ...
            renderer = QgsCategorizedSymbolRenderer(
                "label", [cat_start, cat_end, cat_normal]
            )
...
def process_flight_logic(
    ...
    try:
        ...
        is_geo = source.sourceCrs().mapUnits() == QgsUnitTypes.DistanceDegrees
        conversion_factor = 111320.0 if is_geo else 1.0
        ...
        if use_custom_angle:
            ...
            feedback.pushInfo(
                f"ℹ️ Orientation: Custom azimuth of {azimuth_val}° "
                f"(Map rotation: {angle_deg}°)."
            )
        else:
        ...
        while y > box.yMinimum():
            x_min = box.xMinimum() - (1.0 / conversion_factor)
            x_max = box.xMaximum() + (1.0 / conversion_factor)
            grid_line = QgsGeometry.fromPolylineXY([
                QgsPointXY(x_min, y),
                QgsPointXY(x_max, y)
            ])
        ...
        for line_geom in clipped_lines:
            ...
            if len(points) >= 2:
                ...
                for i in range(num_segments):
                    frac = i / num_segments
                    x_inter = p_start.x() + dx * frac
                    y_inter = p_start.y() + dy * frac
        ...
        if dem_layer and source.sourceCrs() != dem_layer.crs():
            transform_crs_dem = QgsCoordinateTransform(
        ...
        for pt in waypoints:
            ...
            if dem_layer:
                v = read_elevation(pt)
                if v is not None:
                    relative_v = v - base_altitude_start
                    final_height = int(round(relative_v + flight_altitude))
        ...
        if sink_points is not None:
            ...
            for i, pt_data in enumerate(waypoints_with_height):
                ...
                pt_geom = QgsGeometry.fromPointXY(
                    QgsPointXY(pt_data[0], pt_data[1])
                )
                pt_feat.setGeometry(pt_geom)
                lbl = "Start" if i == 0 else ("End" if i == total_pts - 1 else "")
                pt_feat.setAttributes([
                    i, pt_data[2], flight_speed, "Smooth Flight", lbl
                ])
        ...
        line_geo = QgsGeometry.fromPolylineXY(waypoints)
        total_dist_meters = dist_calc.measureLength(line_geo)
        ...
        if total_time_min > MAX_FLIGHT_TIME:
            feedback.pushWarning(
                f"🚨 DANGER: Estimated flight time ({total_time_min:.1f} min) "
                f"exceeds safe limit of {MAX_FLIGHT_TIME} min!"
            )

        feedback.pushInfo("\n" + "=" * 45)
        feedback.pushInfo("📊 FLIGHT REPORT (FIELD ESTIMATE)")
        feedback.pushInfo(f"   Total Route Distance: {total_dist_meters:.2f} m")
        feedback.pushInfo(f"   Transitions (Border curves): {num_curves}")
        feedback.pushInfo(f"   Cruising Speed: {flight_speed} m/s")
        feedback.pushInfo(f"   Estimated Time: {minutes} min, {seconds} sec")
        ...
        if kmz_path and kmz_path != 'TEMPORARY_OUTPUT':
            ...
            if source.sourceCrs().authid() != "EPSG:4326":
                ...
                transform_wgs84 = QgsCoordinateTransform(
                    source.sourceCrs(), wgs84_crs, context.transformContext()
                )
...
<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:wpml="http://www.dji.com/wpmz/1.0.2">
  <Document>
    <wpml:author>QGIS_GeoLogic</wpml:author>
...
class DjiSimpleWaypointAlgorithm(QgsProcessingAlgorithm):
    ...
    def createInstance(self):
        return DjiSimpleWaypointAlgorithm()

    def name(self):
        return 'export_dji_wpml_simple'

    def displayName(self):
        return self.tr('Generate DJI Mission (Simple)')

    def group(self):
        return self.tr('GeoLogic Scripts')

    def groupId(self):
        return 'geologicscripts'

    def shortHelpString(self):
        return self.tr(
            "Express mode for quick and safe flight planning. Automatically handles "
            "map rotation, dynamic densification, and buffer safety for mapping missions."
        )

    def helpString(self):
        return self.tr(
            "<h3>GeoLogic Simple Flight Planner</h3>"
            "<p>This tool quickly generates an optimal flight plan for DJI drones using a selected boundary polygon. "
            "It automatically calculates the best flight line orientation to maximize battery efficiency.</p>"
            "<h4>Parameters:</h4>"
            "<ul>"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo(
            f"ℹ️ Express Mode: Auto-spacing = {spacing_val:.2f}m. "
            f"Auto-buffer = {buffer_val:.2f}m."
        )

        kmz_path = self.parameterAsFileOutput(
            parameters, self.OUTPUT_KMZ, context
        )
...
class DjiAdvancedWaypointAlgorithm(QgsProcessingAlgorithm):
    ...
    def createInstance(self):
        return DjiAdvancedWaypointAlgorithm()

    def name(self):
        return 'export_dji_wpml_advanced'

    def displayName(self):
        return self.tr('Generate DJI Mission (Advanced)')
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "overlap calculation, native CRS formatting, and advanced hardware memory protection."
        )

    def helpString(self):
        return self.tr(
            "<h3>GeoLogic Advanced Flight Planner</h3>"
            "<p>This tool offers granular control over your DJI flight mission parameters. "
            "It is designed for complex mapping environments where precise line direction, custom line spacing, "
            "and terrain densification thresholds are necessary.</p>"
            "<h4>Key Advanced Parameters:</h4>"
            "<ul>"
            "<li><b>Custom Azimuth:</b> Force a specific geographic direction for flight lines (e.g., 0 for North-South flights).</li>"
            "<li><b>Manual Line Spacing:</b> Overrides the automatic photogrammetry overlap calculator to allow arbitrary distances between lines.</li>"
            "<li><b>Boundary Buffer:</b> Creates an external safety margin around your polygon so the drone captures the absolute borders flawlessly.</li>"
            "<li><b>Min Densification Distance:</b> The minimum allowed distance between intermediate waypoints. Lower values (like 5m) generate smoother terrain following but increase waypoint count. The algorithm will dynamically increase this distance if the route exceeds your hardware's <b>Max Waypoints Limit</b>.</li>"
            "</ul>"
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if use_overlap:
            ...
            spacing_val = footprint_width * (1.0 - (overlap_percent / 100.0))
            feedback.pushInfo(
                f"ℹ️ Auto-Spacing: Footprint width is {footprint_width}m. "
                f"Calculating {overlap_percent}% overlap -> Line Spacing = "
                f"{spacing_val:.2f}m"
            )
        else:
            spacing_val = self.parameterAsDouble(
                parameters, self.SPACING, context
            )
            if spacing_val <= 1.0:
                spacing_val = 30.0
            feedback.pushInfo(
                f"ℹ️ Manual Spacing Mode: Using fixed {spacing_val}m line spacing."
            )

        use_custom_angle = self.parameterAsBool(
            parameters, self.USE_CUSTOM_ANGLE, context
        )
        azimuth_val = self.parameterAsDouble(
            parameters, self.CUSTOM_ANGLE, context
        ) if use_custom_angle else 0.0

        buffer_val = self.parameterAsDouble(
            parameters, self.BUFFER_AREA, context
        )
        dens_val = self.parameterAsDouble(
